...
describe("locale files", () => {
  ...
  it.each(Object.entries(LOCALES))(
    ...
    (code, locale) => {
      ...
      if (declaredUntranslated) {
        ...
        expect(status, `${code}: marker must name the source of truth`).toMatch(/GLOSSARY/);
        expect(status, `${code}: marker must forbid machine translation`).toMatch(
          /[Mm]achine translation is not acceptable/,
        );
        return;
      }
  ...
  it("no interface code references an underscore-prefixed key", () => {
    ...
    const files = readdirSync(src, { recursive: true, encoding: "utf8" }).filter(
      (f) => (f.endsWith(".tsx") || f.endsWith(".ts")) && !f.includes("locales.test"),
    );
